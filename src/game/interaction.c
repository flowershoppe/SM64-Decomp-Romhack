...
#ifdef DIALOG_INDICATOR
        struct Object *orangeNumber;
        if (obj->behavior == segmented_to_virtual(bhvCustomPeach) || obj->behavior == segmented_to_virtual(bhvOwl)) {
            orangeNumber = spawn_object_relative(ORANGE_NUMBER_A, 0, 260, 0, obj, MODEL_NUMBER, bhvOrangeNumber);
        } else {
...
void check_death_barrier(struct MarioState *m) {
    if (m->pos[1] < m->floorHeight + 2500.0f) {
